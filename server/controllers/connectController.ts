...
export const connectController: ConnectController = {
  ...
  saveAWSInfo: async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    const { AWSAcessKey, AWSSecretKey, Region }: { AWSAcessKey: string, AWSSecretKey: string, Region: string } = req.body;
    //context format for writing into .env
    const writeENV: string = `AWS_ACCESS_KEY_ID=${AWSAcessKey}\n` +
